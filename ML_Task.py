...
x_axis = df.drop('median_house_value', axis=1).drop('ocean_proximity', axis=1)
y_axis = df['median_house_value']

x_axis_train, x_axis_test, y_axis_train, y_axis_test = train_test_split(x_axis, y_axis, train_size=0.8, random_state=90)
...
def evaluate_mode(model, model_name):
    ...
    #plot_graph(model, y_predictions)
...
def plot_graph(model, y_predictions):
    plt.scatter(x_axis, y_axis)
    plt.plot(x_axis, y_predictions,  color='green', label="ML")
